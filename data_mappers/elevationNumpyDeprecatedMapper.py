# DEPRECATED — numpy/cKDTree per-way elevation mapper.
#
# Superseded by the raster-based ElevationMapper (elevationMapper.py), which
# builds a PostGIS raster in staging, downsamples it with the `raster_aggregate`
# enrichment operator, and samples it along ways at mapping time.
#
# Kept for reference / fallback. NOT wired into config.yaml. Its tables carry a
# `_numpy_deprecated` suffix so they never collide with the active pipeline.
#
# import gc
import os
import threading
...
class ElevationNumpyDeprecatedStagingTable(StagingTable):
    __tablename__ = "elevation_staging_numpy_deprecated"
...
class ElevationNumpyDeprecatedMappingTable(MappingTable):
    __tablename__ = "elevation_mapping_numpy_deprecated"
...
class ElevationNumpyDeprecatedMapper(DataSourceABCImpl):
    ...
    def _flush_metrics_to_staging(self) -> int:
        ...
        ElevationNumpyDeprecatedMapper._metrics.clear()
        return flushed
    ...
    def post_database_processing(self):
        ...
        ElevationNumpyDeprecatedMapper._missing_inside.clear()
        ElevationNumpyDeprecatedMapper._zero_length.clear()
        ElevationNumpyDeprecatedMapper._single_sample.clear()
        ElevationNumpyDeprecatedMapper._tiles_processed = 0
    ...
    def pre_filter_processing(self, result):
        ...
        with ElevationNumpyDeprecatedMapper._flush_lock:
            ElevationNumpyDeprecatedMapper._tiles_processed += 1
            if ElevationNumpyDeprecatedMapper._tiles_processed % ElevationNumpyDeprecatedMapper._FLUSH_EVERY == 0:
                self.logger.info(
                    f"[periodic flush] {ElevationNumpyDeprecatedMapper._tiles_processed} tiles processed — "
                    f"flushing {len(ElevationNumpyDeprecatedMapper._metrics)} way metrics to DB"
                )
