...
| Continuous raster | 1 m DEM tiles | Elevation | Sample along the road at intervals, or compute per-way stats | `sql_template` (profile) or `custom` (stats) | [elevationMapper.py](../data_mappers/elevationPointDeprecatedMapper.py), [elevationPythonMapper.py](../data_mappers/elevationMapper.py) |
